...
void setupWebserverModeAP()
{
  ...
  {
    // [POST] - ROUTE: '/' => Render UI interface
    server_mode_ap.on("/", [](){ server_mode_ap.send(200, "text/html", template_mode_ap); });
    server_mode_ap.on("/reset-config", [](){
      if(server_mode_ap.method() == HTTP_POST) {
        ...
        server_mode_ap.send(403, "application/json", "{\"message\":\"FORBIDDEN\"}");
      }
    });
    // [GET] - ROUTE: '/is-config' => Check WIFI is configuration
    server_mode_ap.on("/is-config", [](){
      bool checkCached = checkHasWifiCached();
      bool isPass = false;
      if(checkCached == true) {
        isPass = true;
